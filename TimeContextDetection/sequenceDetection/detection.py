...
def run_detection_phase(dataset_path, model, tokenizer, device, phase_name, thresholds=None):
    ...
    # Definiamo i nomi di tutte e 7 le colonne presenti nel file
    col_names = ['payload', 'l1', 'l2', 'l3', 'l4', 'l5', 'label']
    # Leggiamo tutto il CSV correttamente
    df = pd.read_csv(dataset_path, header=None, names=col_names, dtype=str)
    # Ora estraiamo solo quello che ci serve: la prima e l'ultima colonna
